			    ...
			    <?php foreach ($presensi->result_array() as $row) { ?>
			    	<tr>
			    		...
			    		<td>
							...
							<button onclick="download_file('<?php echo site_url('kepsek/download_presensi_pegawai/' . $row['id'] . '/' . $_POST['filter_tahun'] . '/' . $_POST['filter_bulan'])?>','<?php echo site_url('kepsek/download_presensi_pegawai/' . $row['id'] . '/' . $_POST['filter_tahun'] . '/' . $_POST['filter_bulan'] . '/xls' )?>')"  class="btn btn-success" data-toggle="tooltip" title="Download">
