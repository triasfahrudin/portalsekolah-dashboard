    ...
    <div class="col-md-10">
			    ...
			    <?php foreach ($presensi->result_array() as $row) { ?>
			    	<tr>
			    		...
			    		<td>
							...
							<a href="<?php echo site_url('dinas/download-presensi-siswa/' . $row['id'] . '/' . $_POST['filter_tahun'] . '/' . $_POST['filter_bulan'])?>" class="btn btn-success" data-toggle="tooltip" title="Download">
		...
		<script type="text/javascript">
		  ...
		  function detail_presensi(var_siswa_id){		    
              $.ajax({
                url : "<?php echo site_url('dinas/detail-presensi-siswa')?>",
                type : "post",
                async: false,
                dataType: 'json',
                data:{
