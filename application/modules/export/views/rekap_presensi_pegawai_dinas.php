<html>
   ...
   <body>
   <table cellspacing=0 cellpadding=0 width="79%" style='width:79.28%;margin-left:49.65pt;border-collapse:collapse;mso-border-bottom-alt:solid windowtext 3.0pt; mso-yfti-tbllook:1184;mso-padding-alt:0cm 5.4pt 0cm 5.4pt;mso-border-insideh: none;mso-border-insidev:none'>
    ...
    <br><br>
          ...
          <thead>
            <?php $fields = $rs->list_fields(); ?>
              <tr>
                  ...
                  <th><?php echo strtoupper(strtr($field,array('_' => ' ','.' => '')))?></th>
                  <?php } ?>
              </tr>
          </thead>
           ...
           <?php foreach ($rs->result() as $d) { ?>
              ...
              <?php foreach ($fields as $field) { ?> 
					   <?php 
							$exp = explode('|',$d->$field);
							if(count($exp) > 1){ ?>
								<td style="background-color:<?php echo $exp[1];?>"><?php echo $exp[0]?></td>
							<?php }else{ ?>
		...
		<table style="text-align: left; width: 203px; height: 116px; margin-left: auto; margin-right: 0px;" border="0" cellpadding="2" cellspacing="2">
			<tbody>
				<tr>
					<td style="vertical-align: top; text-align: center;">KEPALA DINAS<br>
					<br>
						<span
