...
class Dinas extends CI_Controller
{
    ...
    public function profile($token_login)
    {
        ...
        try {
			...
			$crud->field_type('kabupaten_id', 'readonly');
            ...
            if ($state === 'insert_validation') {
                ...
                if ($user_id_provided != $user_id) {
                    redirect(site_url('webview/dinas/profile/' . $token_login . '/edit/' . $user_id), 'reload');
                }
    ...
    public function ganti_password($token_login)
    {
        ...
        if (!empty($_POST['pass_lama'])) {
            ...
            $cek_user = $this->db->get_where('user', array('id' => $user_id));
            ...
            if ($cek_user->num_rows() > 0) {
                ...
                if (password_verify($password, $user['password'])) {
                    ...
                    if (empty($_POST['pass_baru']) || empty($_POST['pass_ulangi'])) {
                        ...
                        redirect(site_url('webview/dinas/ganti-password/' . $token_login), 'reload');
                    } else {
