...
<head>
   ...
   <title>Guru - Portal Sekolah</title>
...
<body class="hold-transition sidebar-mini">
      ...
      <nav class="main-header navbar navbar-expand navbar-white navbar-light">
         ...
         <ul class="navbar-nav">
            ...
            <li class="nav-item d-none d-sm-inline-block">
               <a href="<?php echo site_url('guru') ?>" class="nav-link">Beranda</a>
            </li>
            <!-- li class="nav-item d-none d-sm-inline-block">
         ...
         <div class="sidebar">
            ...
            <nav class="mt-2 main-menu">
               <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
                  ...
                  <li class="nav-item">
                     <a href="<?php echo site_url('guru/dokumen') ?>" class="nav-link">
                  ...
                  <li class="nav-item">
                     <a href="<?php echo site_url('guru/kelola-siswa-kelas') ?>" class="nav-link">
                  ...
                  <li class="nav-item">
                     <a href="<?php echo site_url('guru/pengajuan-izin') ?>" class="nav-link">
                  ...
                  <li class="nav-item">
                     <a href="<?php echo site_url('guru/jadwal-mengajar') ?>" class="nav-link">
                        <i class="nav-icon fas fa-book-reader"></i>
                        <p>
                           Jadwal Mengajar
                        </p>
                  ...
                  <li class="nav-item">
                     <a href="<?php echo site_url('guru/profile/edit/' . $user_id) ?>" class="nav-link">
                        <i class="nav-icon fas fa-user-edit"></i>
                        <p>Profil</p>
                     </a>
                  </li>
                  <li class="nav-item">
                     <a href="<?php echo site_url('guru/ganti-password') ?>" class="nav-link">
                        ...
                        <div class="card-body">
                           <?php if (isset($keterangan)) { ?>
                              <div class="alert alert-warning">
                                 <strong class="infinite slow flash delay-1s">INFORMASI : </strong> <?php echo $keterangan; ?>
                              </div>
                           <?php } ?>
                           <?php if (isset($output)) {
                              echo $output;
   ...
   <script>
      // Library jQuery untuk menampilkan jumlah jam, menit dan detik yang tersisa

      (function($) {
         function countDown(options) {
            var targetTime = new Date(options.targetTime);
            var countdownAlertText = options.countdownAlertText;
            ...
            return {
               ...
               minutes: minutes,
               seconds: seconds,
               countdownAlertText: countdownAlertText
            };
         }

         function showCountDown(targetTime, countdownAlertText, element) {
            var countdown = countDown({
               targetTime: targetTime,
               countdownAlertText: countdownAlertText
            });

            $(element).html('<countdown class="countdown" style="white-space: nowrap;"></countdown>');
            ...
            var countdownInterval = setInterval(function() {
               ...
               if (countdown.hours <= 0 && countdown.minutes <= 0 && countdown.seconds <= 0) {
                  ...
                  swal({
                     html: true,
                     title: 'Notifikasi',
                     text: countdown.countdownAlertText,
                     type: 'warning',
                     confirmButtonText: 'Ok'
                  });
                  $(element).html(
                     '<countdown class="countdown">00:00:00</countdown>'
                  );
                  ...
                  var text =  '<countdown class="countdown">' +
      ...
      <?php if (!empty($active_pelajaran)) { ?>

         var now = new Date();
         var activePelajaranTime = new Date('<?php echo $active_pelajaran[0]['jam_selesai'] ?>');
         now.setSeconds(now.getSeconds() + 2);
         if (now > activePelajaranTime) {
            
            var li_active_pelajaran = document.getElementById('li_active_pelajaran');
            if(li_active_pelajaran){
               li_active_pelajaran.remove();
            }
            
         } else {
            $('#countdown1').countDown({
               'targetTime': '<?php echo $active_pelajaran[0]['jam_selesai'] ?>',
               'countdownAlertText': 'Jam pelajaran telah berakhir'
            })
      ...
      <?php if (!empty($next_pelajaran)) { ?>

         var now = new Date();
         var activePelajaranTime = new Date('<?php echo $next_pelajaran[0]['jam_mulai'] ?>');
         now.setSeconds(now.getSeconds() + 2);
         if (now > activePelajaranTime) {
            //var li_next_pelajaran = document.getElementById('li_next_pelajaran');
            //li_next_pelajaran.remove();
         } else {
            $('#countdown2').countDown({
               'targetTime': '<?php echo $next_pelajaran[0]['jam_mulai'] ?>',
               'countdownAlertText': 'Pelajaran selanjutnya dimulai'
            });
